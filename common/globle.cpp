...
CString TypeName[] = {  "普通用户",  "管理员",  "超级管理员" };
CString BookType[] = { "全开", "对开"};
CString StaffType[] = {  "工人",  "师傅" ,  "师傅+工人"};
CString ZyType[] = { "全开四折页", "四折页", "大三折页", "三折页", "二折页"};
CString rkType[] = {"未入","已入"};
CString proNumType[] = {"印数","令数"};
CString proStaffType[] = {"是","否"};
CString DateName[] = {"全部","最近一周", "最近一个月", "最近三个月", "半年内", "一年内"};
...
CGloble::CGloble():m_Client(this)
{
	...
	m_ConfigFilePath = strWorkDir+"/config/config.ini";
}
...
bool CGloble::InitGloble()
{
	...
	{
		TCHAR ip[30];
		GetPrivateProfileString("SOCKET","ip","",ip,30,m_ConfigFilePath);
		m_LocalIP=ip;	
		m_TcpPort=GetPrivateProfileInt("SOCKET","port",0,m_ConfigFilePath);
		TCHAR name[125];
		GetPrivateProfileString("SOCKET","name","",name,125,m_ConfigFilePath);
		m_strName=name;
	}
	_file.close();
	return true;
...
CString CGloble::ReturnBeginTime(EM_DATE_TYPE type)
{
	...
	strBeginTime.Format("%d/%02d/%02d",year,month,day);
	return strBeginTime;
}
...
bool CGloble::ConnectSer()
{
	...
	m_pkgInfo.Reset();
	BOOL bRet = m_Client->Start(m_LocalIP.c_str(), m_TcpPort, m_bAsyncConn);
	if(bRet)
	{
		if (m_heart == INVALID_HANDLE_VALUE)
		{
