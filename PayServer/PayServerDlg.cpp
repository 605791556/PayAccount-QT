...
DWORD WINAPI StartSerThread(LPVOID lparam)
{
	...
	{
		bOk = pData->m_Server->Start(g_Globle.m_LocalIP.c_str(), g_Globle.m_TcpPort);
		if(bOk)
		{
			pData->RunDog();
			::LogServerStart(g_Globle.m_LocalIP.c_str(), g_Globle.m_TcpPort);
			break;
		}
		else
		{
...
LONG WINAPI MyUnhandledExceptionFilter(struct _EXCEPTION_POINTERS *pExceptionPointers)
{
	...
	strDump.Format("%s\\dump\\%02dH%02dM%02dS%02dMS.dmp",strPath,t.wHour,t.wMinute,t.wSecond,t.wMilliseconds);

	USES_CONVERSION;
	HANDLE hFile = CreateFileA(strDump,GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
...
BOOL CPayServerDlg::OnInitDialog()
{
	...
	{
		CString str;
		str.Format("IP：%s    端口：%d",g_Globle.m_LocalIP.c_str(),g_Globle.m_TcpPort);
		m_staIP.SetWindowText(str);

		if(m_Server->Start(g_Globle.m_LocalIP.c_str(), g_Globle.m_TcpPort))
		{
			//RunDog();
			::LogServerStart(g_Globle.m_LocalIP.c_str(), g_Globle.m_TcpPort);
		}
...
void CPayServerDlg::KillDog()
{
	...
	do
	{
...

		{
			...
			{
				if(strstr(pe32.szExeFile,kill[i]))
				{
...
BOOL CPayServerDlg::RunDog()
{
	...
	CString dogPathName = strPath+"\\HoldDog.exe";
	if (!PathFileExists(dogPathName))
	{
		str.Format("看门狗程序启动失败：%s",dogPathName);
		AddString(str);
		return FALSE;
	}
	else
	{
		ShellExecute(NULL,"open",dogPathName,NULL,strPath,SW_SHOWNORMAL);
		AddString("看门狗程序启动成功");
	}
	return TRUE;
}
...
void CPayServerDlg::DoRun(string strData,Json::Value& js,TPkgInfo* pInfo)
{
	try
	{
		...
		case SOCK_CMD_JUDGE_USER:
			{
				CString strName = root[CMD_JUDGEUSER[EM_JUDGE_USER_NAME]].asCString();
				bRet=theApp.m_dbData->_JudgeUser(strName,js);
			}
			break;
		case SOCK_CMD_ADD_USER:
			{
				CString strName = root[CMD_ADDUSER[EM_ADD_USER_NAME]].asCString();
				CString strPwd = root[CMD_ADDUSER[EM_ADD_USER_PWD]].asCString();
		...
		case SOCK_CMD_DEL_USER:
			{
				CString strName = root[CMD_DELUSER[EM_JUDGE_USER_NAME]].asCString();
		...
		case SOCK_CMD_CHECKPWD:
			{
				CString strName = root[CMD_CHECKPWD[EM_CHECKPWD_NAME]].asCString();
				CString strPass = root[CMD_CHECKPWD[EM_CHECKPWD_PWD]].asCString();
				bRet=theApp.m_dbData->CheckPwd(strName,strPass);
			}
			break;
		case SOCK_CMD_MDFPWD:
			{
				CString strName = root[CMD_MDFPWD[EM_MDFPWD_NAME]].asCString();
				CString strPass = root[CMD_MDFPWD[EM_MDFPWD_PWD]].asCString();
				bRet=theApp.m_dbData->ModifyPwd(strName,strPass);
			}
			break;
		case SOCK_CMD_LOGIN:
			{
				CString strUser = root[CMD_LOGIN[EM_LOGIN_NAME]].asCString();
				CString strPass = root[CMD_LOGIN[EM_LOGIN_PASS]].asCString();
				...
				if(m_Server->GetAllConnectionIDs(pIDs,nCount))
				{
					for (int i=0;i<nCount;i++)
					{
						...
						CString strUserd = pInfo->user.c_str();
						if (strUserd == strUser)
						{
							//已经登录
							bLogind = true;
		...
		case SOCK_CMD_GET_BOOK:
			{
				CString strKeyWord = root[CMD_GETBOOK[GETBOOK_KEYWORD]].asCString();
		...
		case SOCK_CMD_DEL_BOOK:
			{
				CString strBookID = root[CMD_DELBOOK[EM_DEL_BOOK_ID]].asCString();
		...
		case SOCK_CMD_JUDGE_BOOK:
			{
				CString strName = root[CMD_JUDGEBOOK[EM_JUDGE_BOOK_NAME]].asCString();
		...
		case SOCK_CMD_MDF_BOOK:
			{
				CString strBookID = root[CMD_BOOKMSG[BOOKMSG_ID]].asCString();
				CString strName = root[CMD_BOOKMSG[BOOKMSG_NAME]].asCString();
				CString strCbs = root[CMD_BOOKMSG[BOOKMSG_CBS]].asCString();
				CString strDate = root[CMD_BOOKMSG[BOOKMSG_DATE]].asCString();
				...
				CString strMsg = root[CMD_BOOKMSG[BOOKMSG_MSG]].asCString();
		...
		case SOCK_CMD_JUDGE_PROJECT:
			{
				CString strProName = root[CMD_JUDGEBOOK[EM_JUDGE_PROJECT_NAME]].asCString();
		...
		case SOCK_CMD_MDF_PROJECT:
			{
				CString strName = root[CMD_PROMSG[EM_PROMSG_NAME]].asCString();
		...
		case SOCK_CMD_GET_PROGRESS:
			{
				CString strBookID = root[CMD_GETPROGRESS[EM_GETPROGRESS_BOOKID]].asCString();
		...
		case SOCK_CMD_GET_STAFF:
			{
				CString strKeyWord = root[CMD_GETSTAFF[EM_GET_STAFF_KEYWORD]].asCString();
		...
		case SOCK_CMD_DEL_STAFF:
			{
				CString strStaffID = root[CMD_DELSTAFF[EM_DEL_STAFF_ID]].asCString();
		...
		case SOCK_CMD_GET_DPAY:
			{
				CString strStaffID = root[CMD_STAFFMSG[EM_STAFF_MSG_STAFFID]].asCString();
				bRet=theApp.m_dbData->GetDPay(strStaffID,js);
			}
			break;
		case SOCK_CMD_JUDGE_STAFF:
			{
				CString strStaffName = root[CMD_JUDGESTAFF[EM_JUDGE_STAFF_IDCARD]].asCString();
		...
		case SOCK_CMD_MDF_STAFF:
			{
				CString strName = root[CMD_STAFFMSG[EM_STAFF_MSG_NAME]].asCString();
				CString strSex = root[CMD_STAFFMSG[EM_STAFF_MSG_SEX]].asCString();
				CString strStaffID = root[CMD_STAFFMSG[EM_STAFF_MSG_STAFFID]].asCString();
				CString strIdCard = root[CMD_STAFFMSG[EM_STAFF_MSG_IDCARD]].asCString();
				CString strTel = root[CMD_STAFFMSG[EM_STAFF_MSG_TEL]].asCString();
				...
				if(cmd == SOCK_CMD_ADD_STAFF)
					bRet=theApp.m_dbData->AddStaff(strName,strSex,strStaffID,strIdCard,strTel,type,sort,fDaypay);
				else if(cmd == SOCK_CMD_MDF_STAFF)
					bRet=theApp.m_dbData->ModifyStaff(strName,strSex,strStaffID,strIdCard,strTel,type,sort,fDaypay);
			}
			break;
...
void CPayServerDlg::OnTimer(UINT_PTR nIDEvent)
{
		...
		if( bRet)
		{
			for (int i=0;i<nCount;i++)
			{
				...
				if (pInfo)
				{
					...
					int day = all_h/24;
					int h = all_h%24;
					str.Format("%d天 %02d:%02d:%02d",day,h,min,sec);
    ...
    str.Format("当前连接数：%d",nCount);
	m_NowSta.SetWindowText(str);
...
EnHandleResult CPayServerDlg::OnAccept(ITcpServer* pSender, CONNID dwConnID, SOCKET soClient)
{
	try
	{
		...
		if(bPass)
		{
			...
			CString str;
			str.Format("%d:%s 连入",dwConnID,szAddress);
			AddString(str);
		}
	...
	try
	{
		...
		if(pInfo != nullptr && pServer != nullptr)
		{
			...
			while(remain >= required)
			{
				...
				{
					CString str;
					str.Format("fetch faild:%d",result);
					AddString(str);
					return HR_ERROR;
				}
			}
...
EnHandleResult CPayServerDlg::OnClose(ITcpServer* pSender, CONNID dwConnID, EnSocketOperation enOperation, int iErrorCode)
{
	...
	CString str;
	str.Format("%d 断开，code：%d",dwConnID,iErrorCode);
	AddString(str);
    return HR_OK;
}
